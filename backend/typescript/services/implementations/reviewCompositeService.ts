...
import IReviewCompositeService from "../interfaces/IReviewCompositeService";
  ...
  function toDTO(reviewedApplicantRecord: ReviewedApplicantRecord): ReviewedApplicantsDTO {
    return {
      applicantRecordId: reviewedApplicantRecord.applicant_record_id,
      reviewStatus: reviewedApplicantRecord.status,
      applicantFirstName: reviewedApplicantRecord.applicant_record.applicant.first_name,
      applicantLastName: reviewedApplicantRecord.applicant_record.applicant.last_name,
    };
  }
  ...
  class ReviewCompositeService implements IReviewCompositeService {
    ...
    async getReviewedApplicantsByUserId(userId: string): Promise<ReviewedApplicantsDTO[]> {
        try {
                ...
                include: [
                    {
                        ...
                        include: [
                            { model: Applicant, attributes: ['id', 'first_name', 'last_name'] },
                        ],
                    },
                ],
            ...
            Logger.error(`Failed to get reviewed applicants by user id. Reason = ${getErrorMessage(error)}`);
            throw error;
        }
    }
